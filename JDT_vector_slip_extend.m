...
% Extend 1 more layer or interpolation
    a = find(depth_km==0.211);
    first_row = [x_km(a'), y_km(a'), -depth_km(a')];
    b = find(depth_km==0.6310);
    second_row = [x_km(b'), y_km(b'), -depth_km(b')];
    for(i = 1:length(a))
        point1 = first_row(i,:);
        point2 = second_row(i,:);
        z3 = 0;
        ...
        % Calculate distance along the line based on Z coordinate difference
        distance = (z3 - point1(3)) / normalized_direction(3);
        % Calculate XY coordinates of the third point
        third_point = point1 + distance * normalized_direction;
        x3(i) = third_point(1);
        y3(i) = third_point(2);
    end

    surface_xyz = [x3', y3', zeros(12,1)*1.0];

    z = [-depth_km; surface_xyz(:,3)];
    y = [y_km; surface_xyz(:,2)];
    x = [x_km; surface_xyz(:,1)];
    values = [slip_cm; slip_cm(a')];
...
% Set title
title('Strong-smoothed Slip Distribution on Fault', 'FontSize', 45, 'FontWeight','bold');
